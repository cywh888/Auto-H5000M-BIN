...
├── build-local.ps1                    # Windows + Docker 一键本地构建并展示产物
├── patches/
...
直接运行根目录脚本即可启用全兼容插件组合、执行本地构建、复制产物并展示关键 manifest：
...
```powershell
.\build-local.ps1 -ConfigOnly          # 只验证配置，停止在下载/编译前
.\build-local.ps1 -SkipFeedsUpdate     # 使用已有 Docker volume，加速本地迭代
.\build-local.ps1 -CleanVolume         # 清空 Docker 构建缓存后重新构建
.\build-local.ps1 -Threads 8
```

成功后产物在 `artifacts/`，脚本会同时显示固件文件、`openwrt-image.manifest` 中的 WiFi / EasyMesh / 中文包关键项，以及 `artifacts.tar.gz`。
